...
def test_supports_navigating_to_page_directly_via_url_path(page: Page, app_port: int):
    """Test that we can navigate to a page directly via URL. using the url_path."""
    page.goto(f"http://localhost:{app_port}/my_url_path")
    wait_for_app_loaded(page)

    expect(page_heading(page)).to_contain_text("Page 8")
